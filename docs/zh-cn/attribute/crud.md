...
| before-open | 弹窗表单打开前的回调，会暂停Dialog的打开，`done`用于打开Dialog，`type`（check/查看，create/新增， update/编辑）为当前窗口的类型，`row`为弹窗表单的绑定的数据新增时为undefined | `function` | `(type, row, done) => void` |
| dialog-close | 弹窗关闭前的回调,type为当前窗口的类型  | `function`  | `(type) => void` |
| permission | 表格多个按钮权限控制，采用函数方式可以精确到行控制 | `CrudPermission` | - |
| search | 搜索变量 | `object` | - |
| page | 分页变量参考Page参数 | `function` | - |
| span-method | 合并行或列的计算方法 | `CrudPageProps` | `({row,column,rowIndex,columnIndex})=>{}` |
...
| layout | 组件布局，子组件名用逗号分隔 | `Array<'prev'\|'pager'\|'next'\|'jumper'\|'->'\|'sizes'\|'total'\|'slot'>` | [total, sizes, prev, pager, next, jumper] |
...
| index | 是否有序号栏 | `boolean` | `true \| false` | `false` |
| indexText | 序号栏文本内容  | `string` | - | 序号 |
| lazy | 表格组件是否懒加载子节点数据 | `boolean` | `true \| false` | `false` |
...
| type | 表单项的类型 | `FormItemType` | `input \| textarea \| inputNumber \| select \| checkbox \| datePicker \| radio \| radioButton \| cascader \| switch \| treeSelect \| upload \| sign \| regionalGrid \| deptSelect \|editTable \| selectPeople` |  `input` |
| span | 表单项的栅列 | `number` | - | 12 |
| align | 对齐方式 | `left \| center \| right` | - | `left` |
| headerAlign | 表头对齐方式， 若不设置该项，则使用表格的对齐方式 默认left | `left \| center \| right` | - | `left` |
...
| formatter |当内容过长被隐藏时显示 | `function` | - | `(row: any, column: TableColumnCtx<Column>, cellValue: any, index: number) => void` |
...
| display | 弹出表单当前项是否显示 | `boolean \| (props: {form: any, column: FormColumnProps[],index: number, _ZtBoxType?: DialogFormType}) => boolean` | - | `true` |
| createDisplay | 表单新增时当前项是否显示 | `boolean \| (props: {form: any, column: FormColumnProps[],index: number, _ZtBoxType?: DialogFormType}) => boolean` | - | `true` |
| updateDisplay | 表单编辑时当前项是否显示 | `boolean \| (props: {form: any, column: FormColumnProps[],index: number, _ZtBoxType?: DialogFormType}) => boolean` | - | `true` |
| checkDisplay | 表单查看时当前项是否显示 | `boolean \| (props: {form: any, column: FormColumnProps[],index: number, _ZtBoxType?: DialogFormType}) => boolean` | - | `true` |
...
| 事件名  | 说明 | 回调参数 |
| ------------- | ------------- | :---: |
| row-save | 新增数据后点击确定触发该事件`(row: any, done: (isClear?: boolean) => void, index: number) => void` | `row`表单绑定的数据，`done`关闭弹窗的函数`isClear`默认true关闭弹窗清空表单，isClear/false就只清除按钮loading效果 |
| row-update | 编辑数据后点击确定触发该事件`(row: any, done: (isClear?: boolean) => void, index: number) => void` | `row`表单绑定的数据，`done`关闭弹窗的函数`isClear`默认true关闭弹窗清空表单，isClear/false就只清除按钮loading效果 |
| row-del | 删除数据后触发该事件`(row: any, index: number) => void, index: number) => void` | `row`表格对应项的数据，`index`表格点击项 |
| row-click | 行点击触发该事件`(row: any, column: any, event: Event) => void` | `row`表格对应项的数据 |
| selection-change | 表格多选触发该事件`(selection: any[]) => void` | `selection`表格选中的数据 |
| current-change | 表格`(currentPage: number)=> void` | `currentPage`表格选中的数据 |
...
| closeDialogForm | 关闭表单弹窗 | - |
| openDialogForm | 打开弹窗`(type: DialogFormType, row?: Row<any>, index?: number) =>void` | `type`弹窗类型check/查看，create/新增，update/修改，`row`表格对应的行数据，index 对应表格的第几行 |
| toggleRowRadio | 用于单选表格，`(id: any) => void` | `id`表格数据里对应的rowKey默认为id |
| toggleRowSelection | 用于多选表格，切换某一行的选中状态，如果使用了第二个参数，则可直接设置这一行选中与否`(row: any, selected: boolean)=> void` | `row`表格对应的行数据，`selected`是否选中 |
| validate | 用于表单弹窗 表单校验的 下个版本即将淘汰请使用validateV2`(callback?: FormValidateCallback \| undefined) => void` | `callback`表单提交后触发的回调函数 |
| scrollToField | 用于表单弹窗 滚动到指定的字段`(prop: FormItemProp) => void` | `prop`column里的prop配置 |
| validateV2 | 表单校验 | - |
| clearValidate | 移除该表单项的校验结果`(props?: Arrayable<FormItemProp>) => void` | `props`是个数组，数组里的每一项是column里的prop配置 |
| validateField | 验证具体的某个字段`(props?: Arrayable<FormItemProp>) => void` | `props`是个数组，数组里的每一项是column里的prop配置 |
...
| page | 分页器插槽 | - |
| form | 表单插槽 | - |
| `prop` | 表格插槽,以[Column](#Column)里的prop做插槽名，例如column: [{prop: 'test'}]则插槽名为`#test` 作用域参数为`{ row, $index }` | - |
| `propSearch` | 搜索栏插槽,以[Column](#Column)里的prop拼接`Search`做插槽名，例如column: [{prop: 'test'}]则插槽名为`#testSearch` 作用域参数为 `{ row }` | - |
| `propForm` | 表单插槽,以[Column](#Column)里的prop拼接`Form`做插槽名，例如column: [{prop: 'test'}]则插槽名为`#testForm` 作用域参数为 `{ row }` | - |
| `propGroupForm` | 表单分组插槽,以[Group](#Group)里的prop拼接`Group`做插槽名，例如group: [{prop: 'test'}]则插槽名为`#testGroupForm` | - |
| `propTabForm` | 表单tab插槽,以ViewTabs里的value拼接`TabForm`做插槽名，例如viewTabs: [{value: 'test'}]则插槽名为`#testTabForm` | - |

[form-column]: http://www.aqqxl.top/ZtForm/index.html#Column-共用属性

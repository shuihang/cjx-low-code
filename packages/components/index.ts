// components
export { XCrud } from './crud'
export { XDialog, $XDialog } from './dialog'
export { XEditTable } from './editTable'
export { XForm, XSchemaField } from './form'
export { XConfigProvider, provideGlobalConfig } from './config-provider'

// types
export type { TableOption, CrudPageProps, CrudPermission, BeforeOpen, ColumnProps } from './crud'
export type { DialogProps, DialogDirectiveInterface } from './dialog'
export type { EditTableProps, EditTableOption } from './editTable'
export type {
  FormOption,
  SchemaItem,
  SchemaItemArray,
  FormColumnProps,
  FormItemType,
  FormTypeProps
} from './form'
export type { ConfigProviderContext } from './config-provider'

// plugins
// export * from './infinite-scroll'

...
import type { FormModelValueType, SchemaItemArray, SchemaProvide } from './interface'
...
export default function helpers<F extends object = FormModelValueType>(data: {
  schemaField: SchemaItemArray<F>
  currentField: SchemaProvide<F>
  ...
  return {
     ...
     */
    updateSchemaField: (fields: string[], column: Partial<Omit<SchemaProvide<F>, IgnoreKeys>>) => {
      schemaField.forEach((item, index) => {
        if (fields.includes(item.prop as string)) {

...
export function MeasurePerformance(options: PerformanceMonitorOptions = {}): MethodDecorator {
  // 设置默认配置
  const { threshold = 1000, log = true, report = false, prefix = 'Performance Monitor' } = options
      ...
      try {
        ...
        if (result instanceof Promise) {
          return result.then((res) => {
            ...
            // 输出性能日志
            logPerformance(target, propertyKey, executionTime, threshold, log, prefix)

            // 报告性能数据
            if (report) {
              reportPerformance(target, propertyKey, executionTime)
            }
        ...
        // 报告错误性能数据
        if (report) {
          reportPerformance(target, propertyKey, executionTime, error)
        }
